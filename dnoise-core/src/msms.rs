...
//! individual frame.
//!
//! Public surface: the pure [`combine_and_filter`], the [`MsmsKeepBuilder`] that
//! partitions each ddaPASEF MS/MS frame's points to precursors, and the finished
//! [`MsmsKeep`] that maps the keep sets back onto each frame.
//!
//! **Cross-frame state.** A precursor's keep set depends on every frame that
//! re-isolated it, so all ddaPASEF MS/MS frames must be fed to the builder
//! before any MS/MS frame can be denoised. The keep sets are computed once per
//! run and are read-only afterwards.
...
use crate::windows::{FrameMeta, PasefWindow};
...
/// ddaPASEF MS/MS frames. Built by [`MsmsKeepBuilder`] (or [`build_msms_keep`]).
#[derive(Debug, Clone)]
pub struct MsmsKeep {
...
impl MsmsKeep {
    ...
    pub fn keep_mask(&self, frame: &FlatFrame, frame_id: usize) -> Vec<bool> {
...
/// One precursor's combined fragment points, ready for [`Self::filter`]. Handed
/// out by [`MsmsKeepBuilder::into_spectra`] so callers can filter precursors in
/// parallel.
#[derive(Debug, Clone)]
pub struct PrecursorSpectrum {
    /// Precursor id (index into the keep-set table).
    pub precursor: usize,
    /// Lowest isolation scan over all of this precursor's events.
    pub scan_begin: u32,
    /// Highest isolation scan end (exclusive) over all of its events.
    pub scan_end: u32,
    /// `(scan, tof, intensity)` points from every frame that isolated it.
    pub points: Vec<(u32, u32, u32)>,
}

impl PrecursorSpectrum {
    /// Filter the combined spectrum ([`combine_and_filter`]) and return the kept
    /// keys. Precursor id 0 and empty spectra keep nothing.
    pub fn filter(&self, params: &FilterParams, halo: Option<&HaloParams>) -> HashSet<u64> {
        if self.precursor == 0 || self.points.is_empty() {
            HashSet::new()
        } else {
            let num_local = self.scan_end.saturating_sub(self.scan_begin) as usize;
            combine_and_filter(&self.points, self.scan_begin, num_local, params, halo)
        }
    }
}

/// Accumulates ddaPASEF MS/MS points per precursor across frames.
///
/// Create it from the run's `PasefFrameMsMsInfo` rows, feed it every nonempty
/// MS/MS frame whose id [`Self::wants_frame`] (any order), then finish with
/// [`Self::finish`] (sequential) or [`Self::into_spectra`] +
/// [`MsmsKeepParts::finish`] (filter the spectra in parallel yourself).
#[derive(Debug, Clone)]
pub struct MsmsKeepBuilder {
    sb0: Vec<u32>,
    se0: Vec<u32>,
    frame_windows: HashMap<usize, Vec<(u32, u32, u32)>>,
    raw: Vec<Vec<(u32, u32, u32)>>,
}

/// The frame-to-precursor map of a finished [`MsmsKeepBuilder`], waiting for
/// the per-precursor keep sets.
#[derive(Debug, Clone)]
pub struct MsmsKeepParts {
    frame_windows: HashMap<usize, Vec<(u32, u32, u32)>>,
}

impl MsmsKeepParts {
    /// Assemble the [`MsmsKeep`] from the keep sets, indexed by precursor id
    /// (the order [`MsmsKeepBuilder::into_spectra`] returned).
    pub fn finish(self, keep: Vec<HashSet<u64>>) -> MsmsKeep {
        MsmsKeep {
            keep,
...
impl MsmsKeepBuilder {
    /// Index the isolation events by frame and precursor.
    pub fn new(windows: &[PasefWindow]) -> Self {
        ...
        Self {
            ...
            se0,
            frame_windows,
            raw: vec![Vec::new(); max_prec + 1],
        }
    }

    /// True when frame `Frames.Id` holds isolation events (read and add it).
    pub fn wants_frame(&self, frame_id: usize) -> bool {
        self.frame_windows.contains_key(&frame_id)
    }

    /// Partition one MS/MS frame's points to the precursors isolated in it.
    /// Frames without isolation events are ignored.
    pub fn add_frame(&mut self, frame_id: usize, frame: &FlatFrame) {
        let Some(ws) = self.frame_windows.get(&frame_id) else {
            return;
        };
        for k in 0..frame.len() {
            let s = frame.scan[k];
            if let Some(&(_, _, prec)) = ws.iter().find(|&&(sb, se, _)| s >= sb && s < se) {
                self.raw[prec as usize].push((s, frame.tof[k], frame.intensity[k]));
            }
        }
    }

    /// Split into per-precursor spectra (indexed by precursor id) and the parts
    /// that turn their keep sets into an [`MsmsKeep`].
    pub fn into_spectra(self) -> (Vec<PrecursorSpectrum>, MsmsKeepParts) {
        let spectra = self
            .raw
            .into_iter()
            .enumerate()
            .map(|(p, points)| PrecursorSpectrum {
                precursor: p,
                scan_begin: self.sb0[p],
                scan_end: self.se0[p],
                points,
            })
            .collect();
    ...
    /// Filter every precursor sequentially and build the [`MsmsKeep`].
    pub fn finish(self, params: &MsmsFilterParams, halo: Option<&HaloParams>) -> MsmsKeep {
        let fp = params.as_filter_params();
        let (spectra, parts) = self.into_spectra();
        parts.finish(spectra.iter().map(|s| s.filter(&fp, halo)).collect())
    }
}

/// Build per-precursor keep sets sequentially: read each ddaPASEF MS/MS frame
/// once through `read_frame` (0-based frame index), partition its points to
/// precursors, combine across frames, and filter each precursor's combined
/// spectrum. `meta` is the run's frame list; frames with no peaks or no
/// isolation events are not read.
pub fn build_msms_keep(
    meta: &[FrameMeta],
    windows: &[PasefWindow],
    params: &MsmsFilterParams,
    halo: Option<&HaloParams>,
    mut read_frame: impl FnMut(usize) -> Result<FlatFrame>,
) -> Result<MsmsKeep> {
    ...
    for (i, m) in meta.iter().enumerate() {
        ...
        builder.add_frame(m.id, &flat);
    }
    Ok(builder.finish(params, halo))
}
