...
//! count is a 1/K0 span that depends on the mobility ramp. [`equivalents`] converts
//! each active raw-unit parameter once per run so the log and provenance say
//! what the numbers meant for that run.

use crate::convert::{ConvertableDomain, Tof2MzConverter};
...
mod tests {
    use super::*;
    use crate::convert::Scan2ImConverter;
