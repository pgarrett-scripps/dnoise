...
[Load raw points]                  (scan, mz_idx, intensity, mz, ook0)
   │
   ▼
[Stage 1: Vertical-IM feature filter]   ← iterates num_iterations times
   │
...
4. **Find gap-closed runs**: walk the sorted occupied-scan list. Two
   ...
   exceeds `max_internal_gap` empty scans. Equivalently, break where
   `diff > max_internal_gap + 1` (the diff between consecutive occupied scan
...
new_intensity = mean(
    ...
    where |Δscan|   ≤ smooth_box_scan
      AND |Δmz_idx| ≤ smooth_box_mz_idx
)
```
...
the 3×3 cell neighborhood (guaranteed to contain every point within the
tolerance, because cell side equals tolerance).
...
| `smooth_box_scan` | int (scans) | ±scan window for averaging |
| `smooth_box_mz_idx` | int (TOF idx) | ±mz_idx window for averaging |
...
for p in points (descending intensity order):
    ...
    for ds, dm in product(-1..1, -1..1):
        for q in grid[p.scan_cell + ds, p.mz_cell + dm]:
            if |p.scan - q.scan| > box_scan: continue
            if |p.mz - q.mz| > box_mz_idx: continue
            d = manhattan(p, q)
...
`min_centroid_total` are dropped. This is a final coarse filter for very
...
| Field | Meaning |
|---|---|
| `num_seeds_emitted` | Centroids in final output (post `min_centroid_total`) |
...
| Name | Type | Role |
|---|---|---|
| `box_scan` | int (scans) | NN reach + seed exclusion on the scan axis |
| `box_mz_idx` | int (TOF idx) | Same on the TOF-index axis |
| `min_seed_intensity` | float | Floor for promoting orphans to new seeds |
| `min_centroid_total` | float | Drop final centroids whose summed group intensity falls below this |
...
- Raise `min_window_intensity` (use the feature-span-intensity histogram for
  calibration).
- Raise `min_feature_intensity` (also calibrated against the histogram).
...
- Raise `min_centroid_total` (drop weak groups post-aggregation).
...
| **Box** | A rectangular tolerance region `[−box_scan, +box_scan] × [−box_mz_idx, +box_mz_idx]` around a point. Used by smoothing and the centroider. |
